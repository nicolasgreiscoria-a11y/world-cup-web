/**
 * FIFA World Cup 2026 third-place crossing table.
 *
 * The 8 group winners from groups A, B, D, E, G, I, K, L each face one
 * advancing third-place team. Which third plays which winner depends on
 * which 8 (of 12) groups supplied their third-place finisher.
 *
 * WINNER_GROUPS order matches the combinations table column order:
 *   index 0 → Group A winner, 1 → B, 2 → D, 3 → E, 4 → G, 5 → I, 6 → K, 7 → L
 */

export const WINNER_GROUPS = ["A", "B", "D", "E", "G", "I", "K", "L"] as const;

/**
 * Key   = 8 group letters (sorted A-Z) whose thirds advanced, joined into one string
 * Value = 8-element array; value[i] is the group letter whose third plays WINNER_GROUPS[i]
 */
export const THIRD_COMBINATIONS: Record<string, string[]> = {
...
export interface AdvancingThirdWithGroup {
  teamId: string;
  groupName: string;
}

/**
 * Given the 8 advancing third-place teams (with their group names),
 * returns the teamId array aligned to WINNER_GROUPS order, plus the lookup key.
 */
export function resolveThirdsAssignment(
  advancingThirds: AdvancingThirdWithGroup[]
): { assignment: (string | null)[]; key: string | null } {
  if (advancingThirds.length < 8) {
    return { assignment: Array(8).fill(null), key: null };
  }
  const key = [...advancingThirds].map((t) => t.groupName).sort().join("");
  const groupOrder = THIRD_COMBINATIONS[key];
  if (!groupOrder) {
    return { assignment: Array(8).fill(null), key };
  }
  const byGroup: Record<string, string> = {};
  for (const t of advancingThirds) byGroup[t.groupName] = t.teamId;
  return {
    key,
    assignment: groupOrder.map((g) => byGroup[g] ?? null),
  };
}
